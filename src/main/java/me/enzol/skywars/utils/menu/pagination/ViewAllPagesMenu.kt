package me.enzol.skywars.utils.menu.pagination

import me.enzol.skywars.utils.Callback
import me.enzol.skywars.utils.menu.Button
import me.enzol.skywars.utils.menu.Menu
import me.enzol.skywars.utils.menu.buttons.BackButton
import me.enzol.skywars.utils.menu.pagination.JumpToPageButton
import me.enzol.skywars.utils.menu.pagination.PaginatedMenu
import java.util.HashMap
import org.bukkit.entity.Player

class ViewAllPagesMenu(private val menu: PaginatedMenu) : Menu() {

    init {
        autoUpdate = true
    }

    override fun getTitle(player: Player): String {
        return "Jump to page"
    }

    override fun getButtons(player: Player): Map<Int, Button> {
        val buttons = HashMap<Int, Button>()

        buttons[0] = BackButton(object : Callback<Player> {
            override fun call(value: Player) {
                menu.openMenu(player)
            }
        })

        var index = 10
        for (i in 1..menu.getPages(player)) {
            buttons[index++] = JumpToPageButton(i, menu)
            if ((index - 8) % 9 == 0) {
                index += 2
            }
        }

        return buttons
    }

}